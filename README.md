...
jvm在启动后，默认延迟4秒后启动偏向锁（可以设置jvm参数修改），后续所有新建对象的对象头中，锁状态默认为偏向锁（101）。

如果给锁状态为无锁的对象（001）添加synchronized关键字，对象会直接从无锁（001）升级为轻量级锁（100）。
...
cas竞争失败，多线程自旋，线程id进入等待队列，轻量级锁升级为重量级锁（110)
